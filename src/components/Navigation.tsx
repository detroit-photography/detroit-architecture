    ...
    <nav className="bg-detroit-green text-white sticky top-0 z-50" aria-label="Main navigation">
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex justify-between items-center h-16">
          {/* Logo */}
          <Link href="/architecture" className="flex items-center gap-3" aria-label="Detroit Architecture Repository - Home">
            <span className="font-display text-2xl tracking-wide text-detroit-gold">Detroit Architecture</span>
            <span className="text-white/70 text-sm font-body uppercase tracking-widest hidden sm:inline" aria-hidden="true">Repository</span>
          </Link>

          {/* Desktop Navigation */}
          <div className="hidden md:flex items-center gap-6">
            <Link href="/architecture" className="text-sm uppercase tracking-wider hover:text-detroit-gold transition-colors">
              Buildings
            </Link>
            <Link href="/architecture/map" className="text-sm uppercase tracking-wider hover:text-detroit-gold transition-colors flex items-center gap-1">
            ...
            {/* Cart icon */}
            <Link href="/architecture/store/cart" className="relative p-2 hover:text-detroit-gold transition-colors" aria-label={`Shopping cart${totalItems > 0 ? `, ${totalItems} items` : ''}`}>
        ...
        {isOpen && (
          <div className="md:hidden pb-4 space-y-2 border-t border-detroit-gold/20 pt-4">
            <Link href="/architecture" className="block py-2 text-sm uppercase tracking-wider hover:text-detroit-gold" onClick={() => setIsOpen(false)}>Buildings</Link>
            <Link href="/architecture/map" className="block py-2 text-sm uppercase tracking-wider hover:text-detroit-gold" onClick={() => setIsOpen(false)}>Map</Link>
            <Link href="/architecture/store" className="block py-2 text-sm uppercase tracking-wider hover:text-detroit-gold" onClick={() => setIsOpen(false)}>Print Store</Link>
